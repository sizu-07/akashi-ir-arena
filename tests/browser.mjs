...
try {browser=await chromium.launch({channel:'msedge',headless:true});const context=await browser.newContext({viewport:{width:1180,height:820},hasTouch:true});const admin=await context.newPage();admin.on('pageerror',e=>errors.push(e.message));await admin.goto(base);await admin.waitForFunction(()=>document.querySelector('[data-ticket-destination="operator"]').href.startsWith('https://tickets.example.test/operator?game='));await admin.locator('#pin').fill(config.operatorPin);await admin.getByRole('button',{name:'ログイン',exact:true}).click();await admin.locator('#dashboard').waitFor({state:'visible'});
 ...
 assert.deepEqual(await admin.locator('.match-actions button').evaluateAll(buttons=>buttons.map(button=>button.id||button.dataset.media||button.dataset.action)),['newMatch','syncTicketMembers','video','start']);
 await display.waitForFunction(()=>document.getElementById('video').currentTime>1);
 await admin.locator('[data-video=pause]').click();await display.waitForFunction(()=>document.getElementById('video').paused);
 const stoppedAt=await display.locator('#video').evaluate(v=>v.currentTime);await display.waitForTimeout(400);
 assert.ok(Math.abs(await display.locator('#video').evaluate(v=>v.currentTime)-stoppedAt)<.05,'動画停止中は位置を保持する');
 await admin.locator('[data-video=play]').click();await display.waitForFunction(t=>{const v=document.getElementById('video');return !v.paused&&v.currentTime>t+.2},stoppedAt);
 await admin.locator('[data-video=restart]').click();await display.waitForFunction(()=>{const v=document.getElementById('video');return !v.paused&&v.currentTime<.7});
