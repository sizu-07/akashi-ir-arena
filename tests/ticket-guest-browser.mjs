...
try {
  ...
  assert.equal(await page.getByLabel('4人目のチーム').inputValue(), '');
  assert.equal(await page.locator('#registerForm').evaluate(form => form.checkValidity()), false);
  await page.getByLabel('1人目のチーム').selectOption('A');
  await page.getByLabel('2人目のチーム').selectOption('A');
  assert.equal(await page.getByLabel('3人目のチーム').locator('option[value="A"]').evaluate(option => option.disabled), true);
  await page.getByLabel('3人目のチーム').selectOption('B');
  await page.getByLabel('4人目のチーム').selectOption('B');
  assert.equal(await page.getByLabel('1人目のチーム').locator('option[value="B"]').evaluate(option => option.disabled), true);
  await page.getByLabel('4人目のチーム').selectOption('');
  assert.equal(await page.getByLabel('1人目のチーム').locator('option[value="B"]').evaluate(option => option.disabled), false);
  await page.getByLabel('1人目のチーム').selectOption('B');
  await page.getByLabel('4人目のチーム').selectOption('A');
  assert.deepEqual(await page.locator('#nicknameFields select').evaluateAll(selects => selects.map(select => select.value)), ['B', 'A', 'B', 'A']);
  await page.screenshot({path: 'artifacts/guest-ui/register-teams.png', fullPage: true});
